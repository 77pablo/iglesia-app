...
### 👉 POR DÓNDE RETOMAR (al 28 jul 2026, todo desplegado y 242 tests en verde)
...
- **Guardar acordes de una canción da 400** (bug preexistente, descubierto el 28 jul al inventariar los campos-URL): `guardarLetraCancion` en `web/app.js` reenvía `enlace: c.enlace` y `autor: c.autor`, que son `null` en las canciones sembradas, y `z.string().optional()` rechaza `null` (acepta ausente o cadena, no nulo). Arreglo de una línea: o `nullable()` en `editarCancionSchema`, o que el frontend mande `''`.
