...
1. ✅ ~~**Corregir el nombre de una persona**~~ — **HECHO, pero SOLO en la rama local `feat/corregir-nombre`: sin fusionar a `main` y sin subir.** Spec `docs/superpowers/specs/2026-07-31-corregir-nombre-design.md`, plan `docs/superpowers/plans/2026-07-31-corregir-nombre.md`. Suite en **523 tests** (partió en 509; medido con `cd backend && npm test`).
   ...
   > ⚠️ **Dos pendientes que salieron de la revisión de esta rama, no previstos en el plan:**
   > 1. El nombre del pie de la barra lateral **no se refresca** al corregirlo: solo se asigna al iniciar sesión (`web/app.js:417`), así que hace falta recargar para verlo. No es una regresión de esta rama —ese elemento nunca se repinta— pero ahora es visible, porque antes el nombre no se podía cambiar.
   > 2. Corregir un nombre **no actualiza una prédica ya guardada.** `predica.predicador` es texto libre (puede nombrar a un invitado sin cuenta), así que no es una copia que sincronizar — pero eso se ve en el **portal público** y va a parecer un fallo.
