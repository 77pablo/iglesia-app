...
2. ✅ ~~**Fuente del gasto (Organización).**~~ — **HECHO, pero SOLO en la rama local `feat/fuente-del-gasto`: sin fusionar a `main` y sin subir.** No repitas de memoria si ya se fusionó — compruébalo con `git log main..feat/fuente-del-gasto --oneline` (si sigue mostrando commits, sigue sin fusionar) y con `git log origin/main..main --oneline` para lo ya sabido de `main`. Spec `docs/superpowers/specs/2026-07-31-fuente-del-gasto-design.md`, plan `docs/superpowers/plans/2026-07-31-fuente-del-gasto.md`. Suite en **503 tests** (partió en 471; medido con `cd backend && npm test`, el número del plan y el de esta misma sección habían quedado atrás más de una vez — no te fíes de ningún 495 que veas por ahí).
   ...
   > 6. **Corregir un gasto y auditarlo no van en una transacción** (`organizacion.js`, `PATCH` de `editar_gasto`): el `UPDATE` y el `auditar()` son dos pasos sueltos. Si `auditar()` fallara, la corrección queda aplicada sin rastro — justo lo que esa función promete que no pasa. Es la convención del resto del repo (nada de este módulo usa transacciones), por eso se dejó así a propósito.
   > 7. **Rótulo falso mientras el directorio aún no cargó.** Si se toca ✏️ sobre el gasto de una persona **activa** antes de que `/directorio` haya respondido, el desplegable no la encuentra entre sus opciones todavía, le inyecta una entrada rotulada *"(cuenta inactiva)"* para no perder de vista quién es — y cuando el directorio llega, su opción real también aparece: la persona sale **dos veces**, una con el rótulo equivocado. No mueve dinero (es solo la opción preseleccionada), pero es un rótulo falso en una pantalla de plata.
