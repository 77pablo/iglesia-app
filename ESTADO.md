...
### 🌿 Ramas de trabajo pendientes de integrar (20 jul 2026, sin fusionar a `main`)
- **`feat/mensajeria-chat`** — Chat interno completo (11 tareas + fix, 21/21 tests). Ver **Fase 6** (documentada en el `ESTADO.md` de esa rama). Backend `mensajes.js`+`sse.js`, frontend "Mensajes" con SSE.
- **`feat/seguridad`** (worktree `app-seguridad`) — Hardening: `helmet` (CSP), `express-rate-limit`, validación `zod` (login/admin/tesorería/cuenta), validación de env + `.env.example`, logging de seguridad. Zod en el resto de routers: pendiente.
- Orden sugerido al integrar: seguridad → `main`, luego chat, **verificando que la CSP siga permitiendo el SSE** (`connect-src 'self'`).
