...
> 4. **Accesibilidad, y es deliberado.** `order` de flexbox separa el orden visual del orden del DOM, y un lector de pantalla lee el DOM: sin más, un encabezado podría anunciarse delante de entradas de OTRO grupo (medido en el menú real del pastor: 6 de sus 19 entradas quedaban bajo un encabezado que no era el suyo). La mitigación es `aria-hidden="true"` en cada encabezado — son puramente visuales, no son headings, no reciben foco, y su posición en el DOM es solo un ancla para el `order` —, así que el lector de pantalla oye las 19 entradas en el orden del `NAV` sin ninguna etiqueta engañosa: igual que antes de agrupar el menú. Queda sin tocar una carencia previa y más amplia, ya anotada en la spec de este trabajo: las entradas del menú son `<div onclick>`, no botones reales, así que tampoco son accesibles por teclado; arreglarlo es otro trabajo.
