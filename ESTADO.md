...
*Última actualización: 29 de julio de 2026 (cerrados los cinco menores del indicador; corregida la línea de Organización v2, que daba por pendiente lo ya construido)*
...
- **Antes de escribir en `movimiento`, probar el papel:** lo que el líder necesita es rendirle cuentas al tesorero, y hoy la card de gastos es `no-print` a propósito (`app.js:3600`). Un segundo imprimible de rendición (gastos, total, quién puso qué) es barato, no toca permisos ni esquema, y puede cubrir el caso real sin integración.
...
- **Los tres detalles cosméticos del imprimible** (Organización, ver arriba; menos de 30 líneas, cero dependencias): el botón dice "🖨️ Imprimir" y quien busca un PDF no lo encuentra → "Imprimir / PDF"; **`<title>` es la cadena fija `Iglesia App` y `app.js` nunca la toca** (0 ocurrencias de `document.title`), así que todo PDF guardado se llama `Iglesia App.pdf` → fijarlo antes de `print()` y restaurarlo en `afterprint`; y la hoja impresa no lleva el nombre de la iglesia ni la fecha, que es lo que identifica el papel pegado en la puerta.
